<template>
  ...
  <el-select v-model="form.type" value-key="id" clearable placeholder="请选择类型">
    ...
    <el-table :data="tableData" border @selection-change="handleSelectionChange" >
      ...
      <el-table-column prop="type" label="类型">
        <template v-slot="scope">
            <span v-if="scope.row.type == 0">留校</span>
            <span v-if="scope.row.type == 1">离校</span>
        </template>
      </el-table-column>
  ...
  <el-form-item label="状态" prop="status">
    ...
    <el-form-item label="居住街道" prop="stayStreet">
      <el-input v-model="form.stayStreet" placeholder="请输入校外居住地址"></el-input>
    </el-form-item>
    ...
    <el-form-item label="返校时间" required>
      <el-col>
        <el-form-item prop="backTime">
          <el-date-picker type="date" placeholder="请输入返校时间" v-model="form.backTime" style="width: 100%;"></el-date-picker>
        </el-form-item>
      </el-col>
    </el-form-item>
...
export default {
    ...
    methods: {
        ...
        async search(){
            ...
            const res = await request.post("/apply/back/getList",form)
            console.log(res.data)
            this.tableData = res.data.backApplies
        ...
        async delBatch(){
            ...
            await request.post("/apply/back/del/batch",this.ids)
            this.$message({
          showClose: true,
          message: '执行成功',
          type: 'success'
        ...
        async delById(id){
            console.log(id)
            await request.delete("/apply/back/"+id)
            this.$message({
          showClose: true,
          message: '执行成功',
          type: 'success'
        ...
        async updateApply(form){
            ...
            await request.put("/apply/back/up",form)
            this.$message({
          showClose: true,
          message: '执行成功',
          type: 'success'
